...
export function scrollIntoViewIfNeeded(element: HTMLElement) {
  ...
  const scrollLeftDelta =
    Math.max(0.0, elementRight - viewportRight) ||
    Math.min(0.0, elementLeft - viewportLeft);
  const scrollTopDelta =
    Math.max(0.0, elementBottom - viewportBottom) ||
    Math.min(0.0, elementTop - viewportTop);
