...
describe("computeRulerPhysicalLength", () => {
  // C1: fewer than two points -> 0
  it("returns 0 for an empty path", () => {
    expect(computeRulerPhysicalLength([], [1, 1, 1])).toBe(0);
  });

  it("returns 0 for a single point", () => {
    expect(computeRulerPhysicalLength([p(3, 4, 5)], [1, 1, 1])).toBe(0);
  });

  // C2: two points -> single segment distance
  it("computes the distance between two points (unit scale)", () => {
    expect(
      computeRulerPhysicalLength([p(0, 0, 0), p(3, 4, 0)], [1, 1, 1]),
    ).toBeCloseTo(5, 6);
  });
  ...
  it("sums all consecutive segment lengths", () => {
    ...
    expect(computeRulerPhysicalLength(points, [1, 1, 1])).toBeCloseTo(17, 6);
  });
  ...
  it("treats a zero-length segment as contributing nothing", () => {
    ...
    expect(computeRulerPhysicalLength(points, [1, 1, 1])).toBeCloseTo(3, 6);
  });
  ...
  it("applies the per-axis physical scale", () => {
    ...
    expect(computeRulerPhysicalLength(points, [10, 5])).toBeCloseTo(20, 6);
