...
describe("computeAxisExtentsNm", () => {
  // C4: returns one entry per dimension
  it("returns one entry per dimension", () => {
    expect(
      computeAxisExtentsNm(p(0, 0, 0), p(1, 2, 3), [1, 1, 1]),
    ).toHaveLength(3);
  });

  // C1: equal points -> all zero
  it("returns all zeros for equal points", () => {
    expect(computeAxisExtentsNm(p(4, 5, 6), p(4, 5, 6), [10, 10, 10])).toEqual([
      0, 0, 0,
    ]);
  });
  ...
  it("uses the absolute per-axis difference", () => {
    ...
    const extents = computeAxisExtentsNm(p(1, 5), p(4, 1), [1, 1]);
  ...
  it("applies the per-axis physical scale", () => {
    ...
    const extents = computeAxisExtentsNm(p(0, 0), p(2, 3), [4, 40]);
  ...
  it("yields 0 for a zero-scale (non-length) axis", () => {
    const extents = computeAxisExtentsNm(p(0, 0, 0), p(2, 5, 9), [4, 0, 4]);
    expect(extents[1]).toBe(0);
  });

  // C3: finite, non-negative
  it("returns finite, non-negative extents", () => {
    const extents = computeAxisExtentsNm(p(-3, 2), p(5, -1), [7, 9]);
