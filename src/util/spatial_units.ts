...
export function formatVolume(volumeInCubicNanometers: number) {
  ...
  const value = volumeInCubicNanometers / unit.lengthInNanometers ** 3;
  return `${value.toPrecision(6)}\u202f${unit.unit}³`;
}

export function formatBoundingBoxVolume(
  pointA: vec3,
  pointB: vec3,
  transform: mat4,
) {
  ...
  const preTransformVolume = Math.abs(
    prod3(vec3.subtract(vector, pointB, pointA)),
  );
  const det = Math.abs(
    mat3.determinant(mat3.fromMat4(mat3.create(), transform)),
  );
