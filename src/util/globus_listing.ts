...
export async function getGlobusEndpointListing(
...
export async function getGlobusPathCompletions(
  credentialsProvider: SpecialProtocolCredentialsProvider,
  enteredUrl: string,
  path: string,
  cancellationToken: CancellationToken,
  ...
  const paths = await getGlobusEndpointListing(
    credentialsProvider,
    path.substring(1),
    "/",
    cancellationToken,
  ...
  return {
    offset: offset + enteredUrl.length + 1,
