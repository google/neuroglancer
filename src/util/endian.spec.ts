...
describe("endian", () => {
  it("swapEndian16", () => {
    const original = Uint16Array.of(0x1122, 0x3344);
    const swapped = Uint16Array.from(original);
    swapEndian16(swapped);
    expect(Array.from(swapped)).toEqual([0x2211, 0x4433]);
  });

  it("swapEndian32", () => {
    const original = Uint32Array.of(0x11223344, 0x55667788);
    const swapped = Uint32Array.from(original);
