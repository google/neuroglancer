...
function volumeInfoEqual(a: VolumeInfo, b: VolumeInfo): boolean {
  return (
    coordinateSpacesEqual(a.dimensions, b.dimensions) &&
    arraysEqual(a.order, b.order) &&
    arraysEqual(a.chunkShape, b.chunkShape) &&
    arraysEqual(a.gridOrigin, b.gridOrigin) &&
    arraysEqual(a.lowerBound, b.lowerBound) &&
    arraysEqual(a.upperBound, b.upperBound) &&
...
function parseVolumeInfo(obj: unknown): VolumeInfo {
  ...
  const gridOrigin = verifyObjectProperty(
    obj,
    "gridOrigin",
    verifyIntegerArray,
  );
  const lowerBound = verifyObjectProperty(
    obj,
    "lowerBound",
    verifyIntegerArray,
  );
  const upperBound = verifyObjectProperty(
    obj,
    "upperBound",
    verifyIntegerArray,
  );
  const dataType = verifyObjectProperty(obj, "dataType", (value) =>
    verifyEnumString(value, DataType),
  );
  return {
    dimensions,
    order,
...
function parseVolumeRequest(obj: unknown): VolumeRequest {
  ...
  const kind = verifyObjectProperty(obj, "kind", (kind) =>
    verifyEnumString(kind, RequestKind),
  );
  switch (kind) {
    case RequestKind.VOLUME_INFO: {
      const coordinateSpace = verifyOptionalObjectProperty(
        obj,
        "dimensions",
        coordinateSpaceFromJson,
      );
    ...
    case RequestKind.VOLUME_CHUNK: {
      const volumeInfo = verifyObjectProperty(
        obj,
        "volumeInfo",
        parseVolumeInfo,
      );
      const chunkGridPosition = verifyObjectProperty(
        ...
        (value) =>
          parseFixedLengthArray(
            new Array<number>(volumeInfo.dimensions.rank),
            value,
            verifyInt,
          ),
      );
...
function getDefaultCoordinateSpace(
  userLayer: UserLayer,
  renderLayer: SliceViewVolumeRenderLayer,
): CoordinateSpace {
  ...
  const copyScalesAndUnits = (
    toLayerDimensions: readonly number[],
  ...
  copyScalesAndUnits(
    transform.localToRenderLayerDimensions,
    userLayer.managedLayer.localCoordinateSpace.value,
  );
  copyScalesAndUnits(
    transform.channelToRenderLayerDimensions,
    renderLayer.channelCoordinateSpace.value,
  );
  copyScalesAndUnits(
...
function getDimensionPermutation(
...
function getChunkToRenderLayerTransform(
  ...
  renderLayerScales: Float64Array,
  requestedScales: Float64Array,
) {
  ...
  if (unpaddedRank !== rank) {
    matrix.copy(
      chunkToLayerTransform.subarray(
...
function checkSourceMatch(
  source: SliceViewSingleResolutionSource<VolumeChunkSource>,
  transform: RenderLayerTransform,
  modelToRequestDimension: readonly number[],
  renderLayerScales: Float64Array,
  requestedScales: Float64Array,
): { translation: number[]; chunkToRequestDimension: number[] } | null {
  const { rank, unpaddedRank } = transform;
  const chunkToLayerTransform = getChunkToRenderLayerTransform(
    source,
    transform,
    renderLayerScales,
    requestedScales,
  ...
  for (let layerDim = 0; layerDim < rank; ++layerDim) {
    for (let chunkDim = 0; chunkDim < rank; ++chunkDim) {
      ...
      if (
        chunkDimSeen[chunkDim] !== undefined ||
        layerDimSeen[layerDim] !== undefined
      ) {
...
export class VolumeRequestHandler extends RefCounted {
  ...
  sendVolumeInfoResponseRequested = new Signal<
    (requestId: string, info: unknown) => void
  >();
  sendVolumeChunkResponseRequested = new Signal<
  ...
  constructor(public viewer: Viewer) {
    ...
    this.registerDisposer(
      viewer.layerManager.layersChanged.add(this.debouncedMaybeHandleRequests),
    );
    this.registerDisposer(
      this.requestState.changed.add(this.debouncedMaybeHandleRequests),
    );
  }
    ...
    for (const request of this.requestState.value) {
      ...
      try {
        ...
        if (request.kind === RequestKind.VOLUME_INFO) {
          this.sendVolumeInfoResponseRequested.dispatch(request.id, {
            error: e.message,
          });
        }
      }
  ...
  private maybeHandleRequest(
    ...
    if (renderLayers.length !== 1) {
      throw new Error(
        `Layer has ${renderLayers.length} render layers, expected exactly 1`,
      );
    }
    ...
    const modelToRequestDimension = getDimensionPermutation(
    ...
    const match = findMatchingSource(
      ...
      transform,
      modelToRequestDimension,
      renderLayerCoordinateSpace.scales,
    ...
    for (let chunkDim = 0; chunkDim < unpaddedRank; ++chunkDim) {
      ...
      lowerBound[requestDim] += Math.floor(
        (source.lowerClipBound ?? spec.lowerVoxelBound)[chunkDim],
      );
      upperBound[requestDim] =
        translation[requestDim] +
    ...
    if (request.kind === RequestKind.VOLUME_INFO) {
      this.sendVolumeInfoResponseRequested.dispatch(
        request.id,
        volumeInfoToJson(info),
      );
      return true;
    }
