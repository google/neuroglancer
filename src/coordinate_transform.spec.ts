    ...
    const boxes: TransformedBoundingBox[] = [
      {
        ...
        box: {
          lowerBounds: Float64Array.of(11, 12),
          upperBounds: Float64Array.of(13, 14),
        },
      },
      {
      ...
      },
      {
        transform: Float64Array.of(1, 2, 3, 4, 5, 6, 7, 8, 9),
        box: {
          lowerBounds: Float64Array.of(11, 13),
          upperBounds: Float64Array.of(13, 14),
        },
      },
      {
        ...
        box: {
          lowerBounds: Float64Array.of(11, 12),
          upperBounds: Float64Array.of(13, 15),
        },
      },
    ];
...
describe("CoordinateSpaceCombiner", () => {
  it("supports retain", () => {
    ...
    expect(
      coordinateSpacesEqual(initialSpace, combiner.combined.value),
    ).toBeTruthy();
    ...
    expect(combiner.dimensionRefCounts).toEqual(
      new Map([
        ["a", 1],
        ["d", 1],
      ]),
  ...
  it("handle renames", () => {
    ...
    const combiner = new CoordinateSpaceCombiner(
      new WatchableValue(emptyInvalidCoordinateSpace),
      () => true,
    );
    combiner.bind(a);
    combiner.bind(b);
  ...
  it("works for identity transform", () => {
    const inputSpace = makeCoordinateSpace({
      ...
      boundingBoxes: [
        {
          box: {
            lowerBounds: Float64Array.of(1, 2),
            upperBounds: Float64Array.of(5, 10),
          },
    ...
    const watchableTransform = new WatchableCoordinateSpaceTransform(
      makeIdentityTransform(inputSpace),
    );
...
describe("homogeneousTransformSubmatrix", () => {
  it("works for rank 2", () => {
    ...
    const permuted = homogeneousTransformSubmatrix(
      Float32Array,
      orig,
      2,
      [0, 1],
...
describe("WatchableCoordinateSpaceTransform.spec", () => {
  it("preserves coordinate arrays from input space with outputDimensions", () => {
    const inputSpace = makeCoordinateSpace({
      ...
      units: ["", ""],
      coordinateArrays: [
        { explicit: false, coordinates: [0], labels: ["x"] },
        undefined,
      ],
    });
    const watchableTransform = new WatchableCoordinateSpaceTransform(
    ...
    expect(watchableTransform.value.inputSpace.coordinateArrays).toEqual(
      inputSpace.coordinateArrays,
    );
    expect(watchableTransform.value.outputSpace.coordinateArrays).toEqual(
      inputSpace.coordinateArrays,
    );
  });
