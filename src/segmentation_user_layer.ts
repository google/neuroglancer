  ...
  implements SegmentationGroupState
{
  ...
  restoreState(specification: unknown) {
    verifyOptionalObjectProperty(
      specification,
      HIDE_SEGMENT_ZERO_JSON_KEY,
      (value) => this.hideSegmentZero.restoreState(value),
    );
    verifyOptionalObjectProperty(
      specification,
      EQUIVALENCES_JSON_KEY,
      (value) => {
  ...
  implements SegmentationColorGroupState
{
  ...
  restoreState(specification: unknown) {
    verifyOptionalObjectProperty(specification, COLOR_SEED_JSON_KEY, (value) =>
      this.segmentColorHash.restoreState(value),
    );
    verifyOptionalObjectProperty(
      specification,
      SEGMENT_DEFAULT_COLOR_JSON_KEY,
      (value) => this.segmentDefaultColor.restoreState(value),
    );
    verifyOptionalObjectProperty(
      specification,
      SEGMENT_STATED_COLORS_JSON_KEY,
      (y) => {
...
export class SegmentationUserLayer extends Base {
  ...
  getLegacyDataSourceSpecifications(
    ...
    if (
      layerSpec[EQUIVALENCES_JSON_KEY] !== undefined &&
  ...
  restoreState(specification: any) {
    ...
    this.displayState.selectedAlpha.restoreState(
      specification[SELECTED_ALPHA_JSON_KEY],
    );
    this.displayState.saturation.restoreState(
      specification[SATURATION_JSON_KEY],
    );
    this.displayState.notSelectedAlpha.restoreState(
      specification[NOT_SELECTED_ALPHA_JSON_KEY],
    );
    this.displayState.hoverHighlight.restoreState(
      specification[HOVER_HIGHLIGHT_JSON_KEY],
    );
    this.displayState.objectAlpha.restoreState(
      specification[OBJECT_ALPHA_JSON_KEY],
    );
    ...
    skeletonRenderingOptions.restoreState(
      specification[SKELETON_RENDERING_JSON_KEY],
    );
    ...
    this.displayState.renderScaleTarget.restoreState(
      specification[MESH_RENDER_SCALE_JSON_KEY],
    );
    ...
    const linkedSegmentationColorGroupName = verifyOptionalObjectProperty(
      specification,
      LINKED_SEGMENTATION_COLOR_GROUP_JSON_KEY,
  ...
  toJSON() {
    ...
    if (
      ...
      x[LINKED_SEGMENTATION_COLOR_GROUP_JSON_KEY] =
        linkedSegmentationColorGroup.toJSON() ?? false;
    }
    x[EQUIVALENCES_JSON_KEY] =
