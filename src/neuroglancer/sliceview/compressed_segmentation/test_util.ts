...
export function makeRandomUint64Array(length: number, numPossibleValues: number) {
  ...
  for (let i = 0; i < length; ++i) {
    let index = Math.floor(Math.random() * numPossibleValues);
...
export function makeRandomUint32Array(length: number, numPossibleValues: number) {
