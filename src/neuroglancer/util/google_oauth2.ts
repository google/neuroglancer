...
export function embedRelayFrame(proxyName: string, rpcToken: string) {
  ...
  iframe.src = `https://accounts.google.com/o/oauth2/postmessageRelay?parent=${encodeURIComponent(
      origin)}#rpctoken=${rpcToken}`;
        ...
        try {
          ...
          if (service === this.oauth2CallbackService) {
            ...
            if (!arg.startsWith(origin + '#') && !arg.startsWith(origin + '?')) {
              throw new Error(`oauth2callback: URL ${JSON.stringify(
                  arg)} does not match current origin ${origin}.`);
            }
            ...
            for (let part of parts) {
              ...
              if (match === null) {
                throw new Error(`oauth2callback: URL part ${JSON.stringify(
                    match)} does not match expected pattern.`);
              }
          ...
          throw new Error(`Invalid message received from ${AUTH_ORIGIN
                          }: ${JSON.stringify(event.data)}: ${parseError.message}.`);
        }
      });
    });
  }
