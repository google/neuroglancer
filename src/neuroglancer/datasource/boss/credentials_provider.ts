...
export class KeycloakService {
  ...
  static init(authServer: string): Promise<any> {
    const keycloakAuth = Keycloak({url: authServer, realm: 'BOSS', clientId: 'endpoint'});

    KeycloakService.auth.loggedIn = false;

    return new Promise((resolve, reject) => {
      keycloakAuth.init({onLoad: 'login-required', responseMode: 'query'})
          .success(() => {
    ...
    return new Promise<BossToken>((resolve, reject) => {
      ...
      function login() {
        ...
        if (KeycloakService.auth.authz && KeycloakService.auth.authz.token) {
          ...
          KeycloakService.init(authServer).then(() => {
            login();
          });
        }
      }
