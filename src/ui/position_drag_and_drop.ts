...
export function setupPositionDropHandlers(
  ...
  const dropDisposer = registerEventListener(
    target,
    "drop",
    (event: DragEvent) => {
      ...
      if (event.dataTransfer!.types.indexOf(positionDragType) !== -1) {
        ...
        const obj = verifyObject(
          JSON.parse(event.dataTransfer!.getData(positionDragType)),
        );
        ...
        const positionVec = verifyObjectProperty(
          obj,
          "position",
          (positionObj) => parseArray(positionObj, verifyFiniteFloat),
        );
  ...
  const dragenterDisposer = registerEventListener(
    target,
    "dragenter",
    handleDragOver,
  );
  const dragoverDisposer = registerEventListener(
    target,
    "dragover",
    handleDragOver,
  );
