...
export function getLayerDragInfo(event: DragEvent): DragInfo | undefined {
  return decodeParametersFromDragTypeList(
    event.dataTransfer!.types,
    layerDragTypePrefix,
  );
}

function getCompatibleDragSource(
  manager: Borrowed<LayerListSpecification>,
): LayerDragSourceData | undefined {
  if (
    dragSource !== undefined &&
    dragSource.manager.rootLayers === manager.rootLayers
  ) {
    return dragSource;
  }
  return undefined;
...
export class DropLayers {
  ...
  initializeExternalLayers(event: DragEvent): boolean {
    ...
    if (dragType !== undefined) {
      try {
        const { layers: spec, layout } = JSON.parse(
  ...
  updateArchiveStates(event: DragEvent) {
    ...
    for (const layer of this.layers.keys()) {
      ...
      if (
        ...
        (shouldBeArchived && layer.visible)
      ) {
        layer.archived = shouldBeArchived;
  ...
  get method() {
    if (this.sourceManager !== undefined) {
      if (
        this.manager === this.sourceManager &&
...
export function getLayerDropEffect(
  ...
  targetIsLayerListPanel: boolean,
  newTarget: boolean,
): { dropEffect: LayerDropEffect; dropEffectMessage: string } {
...
export function updateLayerDropEffect(
  ...
  const result = getLayerDropEffect(
    event,
    manager,
...
export function getDropLayers(
  event: DragEvent,
  manager: Borrowed<LayerListSpecification>,
  options: GetDropLayersOptions,
): DropLayers | undefined {
...
function destroyDropLayers(
  ...
  dropLayers.manager.layerManager.filter(
    (layer) => !dropLayers.layers.has(layer),
  );
...
export function registerLayerBarDropHandlers(
  panel: LayerBarDropInterface,
  target: EventTarget,
  targetLayer: ManagedUserLayer | undefined,
  isLayerListPanel = false,
) {
  function update(
    event: DragEvent,
    updateDropEffect: boolean,
  ):
    | {
        dropLayers: DropLayers;
    ...
    const { dropEffect, dropEffectMessage } = updateDropEffect
      ...
      : {
          dropEffect: getDropEffect() as LayerDropEffect,
          dropEffectMessage: "",
        };
    ...
    if (dropLayers === undefined) {
      dropLayers = panel.dropLayers = getDropLayers(event, panel.manager, {
        forceCopy: dropEffect === "copy",
        newTarget: false,
        isLayerListPanel,
      });
      if (dropLayers === undefined) {
    ...
    if (!existingDropLayers) {
      ...
      if (targetLayer !== undefined) {
        newIndex =
          panel.manager.layerManager.managedLayers.indexOf(targetLayer);
      }
  ...
  target.addEventListener("drop", (event: DragEvent) => {
    ...
    endLayerDrag(
      dropLayers.method === "move" ? undefined : event.dataTransfer!.dropEffect,
    );
  });
  target.addEventListener("dragover", (event: DragEvent) => {
    ...
    if (dropEffect === "none") {
      ...
      const layerCountMessage =
        numSourceLayers === numLayers
          ? `${numSourceLayers}`
