...
export class ToolPaletteState extends RefCounted implements Trackable {
  ...
  tools = this.registerDisposer(new TrackableToolList(this.viewer));
  ...
  queryDefined = this.registerDisposer(
    makeCachedDerivedWatchableValue(
...
export class ToolPalettePanel extends SidePanel {
  ...
  private queryResults = this.registerDisposer(new QueryResults(this.state));
