...
describe("triangle_strips", () => {
  describe("getBaseIndex", () => {
    it("works for simple examples", () => {
      expect([7 * 4 + 0, 7 * 4 + 1, 7 * 4 + 2].map(getBaseIndex)).toEqual([
        7 * 3,
        7 * 3,
        7 * 3,
      ]);
  ...
  describe("getEdgeIndex", () => {
    it("works for simple examples", () => {
      expect([7 * 4 + 0, 7 * 4 + 1, 7 * 4 + 2].map(getEdgeIndex)).toEqual([
        0, 1, 2,
      ]);
    });
  });
    ...
    it("works", () => {
      expect(
        [
          ...
          [1, 1],
          [2, 1],
        ].map(([edgeIndex, flipped]) => getNextEdge(edgeIndex + flipped * 4)),
      ).toEqual([
        2 + 0 * 4,
        2 + 1 * 4,
        1 + 1 * 4,
...
function makeRandomIndices(numTriangles: number, numVertices: number) {
  ...
  for (let i = 0; i < numTriangles * 3; i += 3) {
    while (true) {
      const a = getRandomInt(0, numVertices);
      const b = getRandomInt(0, numVertices);
      const c = getRandomInt(0, numVertices);
...
describe("triangle_strips", () => {
  ...
  it("works for two-strip example with isolated strip", () => {
    ...
    const output = computeTriangleStrips(indices);
    expect(Array.from(output)).toEqual([
      0, 1, 2, 3, 4, 5, 0xffff, 8, 6, 7, 9, 8, 0xffff, 6, 7, 8,
    ]);
  });
  ...
  it("works for random partitioned examples", () => {
    ...
    for (let iter = 0; iter < 10; ++iter) {
      const indices = makeRandomIndices(numTriangles, numVertices);
      const subChunkOffsets = Uint32Array.of(
        0,
        20 * 3,
        (20 + 30) * 3,
        (20 + 30 + 40) * 3,
      ...
      const output = computeTriangleStrips(
        new Uint32Array(indices),
        outputSubChunkOffsets,
      );
      // console.log('indices', Array.from(indices));
      // console.log('output', Array.from(output));
      for (let subChunk = 0; subChunk < 4; ++subChunk) {
        const outputBegin = outputSubChunkOffsets[subChunk];
        const outputEnd = outputSubChunkOffsets[subChunk + 1];
        ...
        const newTriangles = getTrianglesFromStrips(
          output.subarray(outputBegin, outputEnd),
        );
        const origTriangles = getTrianglesFromIndices(
          indices.subarray(
            subChunkOffsets[subChunk],
            subChunkOffsets[subChunk + 1],
          ),
        );
