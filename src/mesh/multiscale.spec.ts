...
function getDrawChunkList(
  ...
  viewportWidth: number,
  viewportHeight: number,
  hasChunk: (row: number) => boolean,
): MultiscaleChunkDrawResult[] {
...
describe("multiscale", () => {
  it("getDesiredMultiscaleMeshChunks simple", () => {
    ...
    const modelViewProjection = mat4.perspective(
      mat4.create(),
      Math.PI / 2,
