...
Angle annotations also share the polyline geometry and are rendered identically to polylines (line segments with endpoint markers), using the same shader API. An angle is a distinct annotation type; the selection panel shows the interior angle at each interior vertex (in degrees), computed in physical space. It is created with the angle tool and behaves like a polyline for rendering purposes.
