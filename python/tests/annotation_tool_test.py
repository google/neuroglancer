...
@pytest.mark.parametrize(
    "tool,tool_class,annotation_class,num_clicks",
    [
        ...
        (
            "annotateBoundingBox",
            neuroglancer.PlaceBoundingBoxTool,
            neuroglancer.AxisAlignedBoundingBoxAnnotation,
            2,
        ),
        (
            "annotateSphere",
            neuroglancer.PlaceEllipsoidTool,
...
def test_annotate(webdriver, tool, tool_class, annotation_class, num_clicks):
    ...
    for i in range(num_clicks):
        ...
        if tool == "annotatePolyline" and i == num_clicks - 1:
            chain.key_down(Keys.ENTER)
            chain.key_up(Keys.ENTER)
